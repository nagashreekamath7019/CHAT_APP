...
const Profile = () => {
    ...
    // Logic to handle gallery selection
    const handleImageChange = (e) => {
        const file = e.target.files[0];
        const MAX_SIZE = 500 * 1024;
        if (file) {
            if (file.size > MAX_SIZE) { 
                return toast.error("File is too large! Max 500KB.");
            }
            const reader = new FileReader();
            reader.onloadend = () => {
                handleUpdateProfile('profilepic', reader.result);
            };
            reader.readAsDataURL(file);
        }
    };

    const handleUpdateProfile = async (field, imageValue = null) => {
        let value = imageValue;
        
        if (field === 'username') value = newUsername;
        if (field === 'fullname') value = newFullname;
        if (field === 'gender') value = newGender;
        ...
        setLoading(true);
        try {
            const endpoint = field === 'profilepic' ? '/api/user/update-profilepic' : `/api/user/update-${field}`;
            ...
            toast.success(`${field === 'profilepic' ? "Profile Picture" : field} updated!`);
            setEditMode(null);
        } catch (error) {
            ...
            <div className='w-full max-w-md bg-white rounded-3xl shadow-xl overflow-hidden'>
                ...
                <div className='flex flex-col items-center p-8'>
                    ...
                    <div className='relative group cursor-pointer' onClick={() => fileInputRef.current.click()}>
                        <div className='w-24 h-24 rounded-full overflow-hidden border-4 border-sky-500 shadow-lg mb-4 bg-gray-200'>
                            {loading && editMode === null ? (
                                <div className="flex items-center justify-center h-full"><span className="loading loading-spinner text-sky-500"></span></div>
                            ) : (
                        ...
                        <div className='absolute inset-0 bg-black/30 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity mb-4'>
                             <BiEditAlt className='text-white' size={24} />
                        </div>
                    </div>
                        ...
                        <div className='bg-gray-50 p-4 rounded-2xl border border-gray-100'>
                            ...
                            {editMode === 'gender' ? (
                                <div className='flex items-center gap-2'>
                                    <select value={newGender} onChange={(e) => setNewGender(e.target.value)} className='bg-white border border-sky-300 rounded-lg p-1 flex-1 outline-none text-sm'>
                    ...
                    {/* MODAL */}
                    {isModalOpen && (
                        <div className='fixed inset-0 flex items-center justify-center z-100 p-4 bg-black/10 backdrop-blur-[2px]'>
                            <div className='bg-white p-6 rounded-3xl shadow-2xl text-center max-w-sm w-full border border-gray-100'>
                                ...
                                <div className='flex justify-center gap-3'>
                                    <button onClick={() => setIsModalOpen(false)} className='flex-1 px-4 py-3 bg-gray-100 text-gray-700 font-bold rounded-2xl hover:bg-gray-200 transition-colors text-sm'>Cancel</button>
                                    <button onClick={handleLogOut} className='flex-1 px-4 py-3 bg-red-500 text-white font-bold rounded-2xl hover:bg-red-600 transition-colors shadow-lg shadow-red-200 text-sm'>
                                        {loading ? "..." : "Logout"}
                                    </button>
                                </div>
                            </div>
